...
# def load_sales_data():
#     engine = init_connection()
#     return pd.read_sql("SELECT * FROM cleaned_sales ORDER BY order_date DESC", engine)

# @st.cache_data
# def load_forecast_data():
#     engine = init_connection()
#     try:
#         return pd.read_sql("SELECT * FROM forecast_results ORDER BY forecast_date", engine)
#     except:
#         return pd.DataFrame()

# def main():
#     st.markdown('<h1 class="main-header">🚀 Sales Forecasting & Analytics Dashboard</h1>', unsafe_allow_html=True)
...
#         regions = ['All'] + sorted(sales_data['region'].dropna().unique().tolist()) if not sales_data.empty else ['All']
#         categories = ['All'] + sorted(sales_data['product_category'].dropna().unique().tolist()) if not sales_data.empty else ['All']
...
#     # Overview KPIs
#     if show_overview and not filtered_data.empty:
#         st.markdown("## 📈 Key Performance Indicators")
#         col1, col2, col3, col4, col5 = st.columns(5)
...
#         col1.markdown(f"""<div class="metric-card"><div class="metric-value">${total_sales:,.0f}</div><div class="metric-label">Total Sales</div></div>""", unsafe_allow_html=True)
#         col2.markdown(f"""<div class="metric-card"><div class="metric-value">{total_orders:,}</div><div class="metric-label">Total Orders</div></div>""", unsafe_allow_html=True)
#         col3.markdown(f"""<div class="metric-card"><div class="metric-value">${avg_order_value:.0f}</div><div class="metric-label">Avg Order Value</div></div>""", unsafe_allow_html=True)
#         col4.markdown(f"""<div class="metric-card"><div class="metric-value">{unique_customers:,}</div><div class="metric-label">Unique Customers</div></div>""", unsafe_allow_html=True)
#         col5.markdown(f"""<div class="metric-card"><div class="metric-value">{unique_products:,}</div><div class="metric-label">Unique Products</div></div>""", unsafe_allow_html=True)

#     # Trends
#     if show_trends and not filtered_data.empty:
#         st.markdown("## 📊 Sales Trends Analysis")
#         col1, col2 = st.columns(2)

#         with col1:
#             daily_sales = filtered_data.groupby('order_date')['sales_amount'].sum().reset_index()
#             fig = px.line(daily_sales, x='order_date', y='sales_amount', title="Daily Sales Trend")
#             st.plotly_chart(fig, use_container_width=True)

#         with col2:
#             regional = filtered_data.groupby('region')['sales_amount'].sum().reset_index()
#             fig = px.bar(regional, x='region', y='sales_amount', title="Sales by Region", color='sales_amount')
#             st.plotly_chart(fig, use_container_width=True)

#         category = filtered_data.groupby('product_category')['sales_amount'].sum().sort_values()
...
#             textposition='outside'
#         ))
#         fig.update_layout(title='Sales Performance by Category')
#         st.plotly_chart(fig, use_container_width=True)
...
#                     (forecast_data['region'] == 'All Regions')
#                 ]
#             if selected_category != 'All':
#                 forecast_data = forecast_data[
#                     (forecast_data['product_category'] == selected_category) | 
#                     (forecast_data['product_category'] == 'All Categories')
#                 ]
...
#                 if not filtered_data.empty:
#                     historical = filtered_data.groupby('order_date')['sales_amount'].sum().reset_index()
#                     fig.add_trace(go.Scatter(x=historical['order_date'], y=historical['sales_amount'], mode='lines', name='Historical Sales', line=dict(color='blue')))

#                 fig.add_trace(go.Scatter(x=forecast_data['forecast_date'], y=forecast_data['predicted_sales'], mode='lines', name='Forecast', line=dict(color='red', dash='dash')))
#                 fig.add_trace(go.Scatter(x=forecast_data['forecast_date'], y=forecast_data['upper_bound'], mode='lines', name='Upper Bound', line=dict(color='rgba(0,0,0,0)'), showlegend=False))
#                 fig.add_trace(go.Scatter(x=forecast_data['forecast_date'], y=forecast_data['lower_bound'], fill='tonexty', mode='lines', name='Confidence Interval', line_color='rgba(0,0,0,0)', fillcolor='rgba(255,0,0,0.2)'))
#                 fig.update_layout(title="Forecast with Confidence Intervals")
#                 st.plotly_chart(fig, use_container_width=True)

#                 st.subheader("📋 Forecast Summary")
#                 summary = forecast_data.groupby(['region', 'product_category']).agg({
#                     'predicted_sales': 'sum',
#                     'accuracy_score': 'mean'
#                 }).reset_index().round(2)
...
#         else:
#             st.info("No forecast data available. Please run the forecasting model.")
...
#         with tab1:
#             top = filtered_data.groupby('product_name').agg({'sales_amount': 'sum', 'quantity': 'sum', 'customer_id': 'nunique'}).reset_index()
#             top.columns = ['Product', 'Total Sales', 'Total Quantity', 'Unique Customers']
#             top = top.sort_values('Total Sales', ascending=False).head(10)
#             st.dataframe(top, use_container_width=True)

#         with tab2:
#             cust = filtered_data.groupby('customer_id')['sales_amount'].agg(['sum', 'count', 'mean']).round(2)
#             cust.columns = ['Total Spent', 'Order Count', 'Avg Order Value']
#             st.dataframe(cust.sort_values('Total Spent', ascending=False).head(10), use_container_width=True)

#         with tab3:
#             filtered_data['day_of_week'] = filtered_data['order_date'].dt.dayofweek
#             filtered_data['month'] = filtered_data['order_date'].dt.month

#             monthly = filtered_data.groupby('month')['sales_amount'].sum()
#             dow = filtered_data.groupby('day_of_week')['sales_amount'].sum()

#             fig1 = px.bar(x=monthly.index, y=monthly.values, title='Monthly Sales Pattern', labels={'x': 'Month', 'y': 'Sales'})
#             fig2 = px.bar(x=['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], y=dow.values, title='Day of Week Sales Pattern')
#             st.plotly_chart(fig1, use_container_width=True)
#             st.plotly_chart(fig2, use_container_width=True)

#     # Footer
#     st.markdown("---")
#     st.markdown("📊 **Sales Forecasting Dashboard** | Built with Streamlit & Plotly | Powered by SQLite")
